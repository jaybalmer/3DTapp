...
export async function GET(
  ...
  try {
    ...
    console.error("Error in GET /api/domains/[slug]/posts:", error)
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message?.includes("relation") || error.message?.includes("does not exist")) {
        return NextResponse.json(
          { error: "Database table not found. Please run the SQL schema from supabase/domain-posts-schema.sql" },
          { status: 500 }
        )
      }
      return NextResponse.json(
    ...
    console.error("Error in POST /api/domains/[slug]/posts:", error)
    const errorMessage = error instanceof Error ? error.message : "Unknown error"
    return NextResponse.json(
      { error: `Internal server error: ${errorMessage}` },
      { status: 500 }
