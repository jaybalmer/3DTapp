...
export default function ProjectDecision({ projectSlug }: ProjectDecisionProps) {
  const [decision, setDecision] = useState<ProjectDecision | null>(null)
  ...
  const fetchDecision = async () => {
    try {
      const response = await fetch(`/api/projects/${projectSlug}/decision`)
      if (response.ok) {
  ...
  const formatBudgetForInput = (value: number): string => {
    // Remove decimals and format as currency input
