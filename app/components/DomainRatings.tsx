...
export default function DomainRatings({ domainSlug }: DomainRatingsProps) {
  const [ratings, setRatings] = useState<DomainRating[]>([])
  const [currentUser, setCurrentUser] = useState<{ email: string; name: string } | null>(null)
  const [loading, setLoading] = useState(true)
  const [submitting, setSubmitting] = useState(false)
  const [userRating, setUserRating] = useState<DomainRating | null>(null)
  ...
  }, [domainSlug, currentUser])

  const fetchRatings = async () => {
    try {
      const response = await fetch(`/api/domains/${domainSlug}/ratings`)
      if (response.ok) {
        ...
        if (user) {
          const userRating = data.find(
            (r: DomainRating) => r.user_email.toLowerCase() === user.email.toLowerCase()
          )
          if (userRating) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `/api/domains/${domainSlug}/ratings/${currentUser.email}`,
        {
          method: "DELETE",
        }
      )
